...
def show_analysis_page(df_batting, df_pitching):
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["📈 チーム傾向", "🆚 対戦相手別", "🔍 詳細投打分析", "🧠 理想オーダー", "🤝 チーム貢献度"])
    ...
    with tab1:
        if df_games.empty:
            ...
            with col_bot2:
                ...
                inn_scores = aggregate_innings(df_b, "得点")  # ユーザー環境のdf_b
                inn_lost = aggregate_innings(df_p, "失点")   # ユーザー環境のdf_p
            ...
            if df_p_detail.empty:
                ...
                if not players_p:
                    ...
                    if my_p.empty:
                        ...
                        # ③ 被安打・四死球の傾向グラフ（打球方向の有無に関わらず、独立して必ず表示）
                        st.write("")
                        st.divider()
    ...
    with tab4:
        ...
        if not df_b.empty:
            ...
            if df_calc.empty:
                ...
                # 2. 基本スタッツの集計
                # 三振データ(is_so)があるか確認し、動的に集計対象に追加する
                agg_dict = {
                    "is_ab": "sum",
                    "is_hit": "sum",
                    "is_bb": "sum",
                ...
                # SO(三振)列が存在しない場合のフォールバック（計算エラー防止）
                if "SO" not in stats.columns:
                    stats["SO"] = 0

                # 3. 最低打数フィルタ (デフォルト: 10打数)
                max_ab = int(stats["AB"].max()) if not stats.empty else 0
                default_ab = min(10, max_ab)
                min_ab = st.slider("対象とする最低打数", 0, max_ab, default_ab)
                stats = stats[stats["AB"] >= min_ab]

                if not stats.empty:
                    # 4. 指標計算
                    stats["PA"] = stats["AB"] + stats["BB"] + stats["SF"]  # 簡易打席数
                    ...
                    stats["K_rate"] = stats.apply(lambda x: x["SO"] / x["PA"] if x["PA"] > 0 else 0, axis=1) # 三振率

                    candidates = stats.copy()

                    # 5. オーダー選出用のカスタム指標算出
                    # 三振率はマイナス評価、盗塁はプラス評価として加味する
                    candidates["Score_1"] = candidates["OBP"] + (candidates["SB"] * 0.02) - (candidates["K_rate"] * 0.5)
                    candidates["Score_3"] = candidates["OPS"] - (candidates["K_rate"] * 0.5)
                    candidates["Score_9"] = candidates["OBP"] + (candidates["SB"] * 0.02)

                    used_players = []
                    lineup = {}

                    # 役割に最も適した選手を「枠」に割り当てる関数
                    ...
                    # 【重要】選出順序（UIの表示順序ではなく、優秀な選手をどの役割から優先確保するかの順番）
                    ...
                    for i in range(1, 10):
                        ...
                        if p is not None:
                            st.success(
                                ...
                                f"打率: {p['AVG']:.3f} | 出塁率: {p['OBP']:.3f} | 長打率: {p['SLG']:.3f} | OPS: {p['OPS']:.3f}\n\n"
    ...
    with tab5:
        st.markdown("### 🤝 チーム貢献度")
        st.caption("「試合に参加すること」は最大の貢献です。出席率とアクティビティを分析し、チームの支柱と状況を把握します。")
        ...
        exclude_list_no_space = [str(n).replace(" ", "").replace(
            " ", "").strip() for n in raw_hidden]
        ...
        if not df_b.empty and not df_games.empty:
            ...
            df_contrib["_clean_name"] = df_contrib["選手名"].astype(
                str).str.replace(r"\s+", "", regex=True)

            # 除外処理
            df_contrib = df_contrib[~df_contrib["_clean_name"].isin(
                exclude_list_no_space)]
            ...
            if df_contrib.empty:
                ...
                df_contrib["BB"] = df_contrib["結果"].isin(
                    ["四球", "死球"]).astype(int)
                df_contrib["AB"] = df_contrib["結果"].isin(
                    hit_cols + ["凡退", "三振", "失策", "併殺打", "野選"]).astype(int)
                df_contrib["TB"] = 0
                ...
                contrib_stats["出席率"] = (
                    contrib_stats["出場試合数"] / total_games_count) * 100

                # ランキング表の作成と表示
                iron_men = contrib_stats.sort_values(
                    ["出場試合数"], ascending=False).head(20)
            ...
            df_all["_clean_name"] = df_all["選手名"].astype(
                str).str.replace(r"\s+", "", regex=True)

            # 有効選手抽出＆除外処理
            all_players = [p for p in df_all["選手名"].unique()
                           if p in valid_numbered_players]
            all_players = [p for p in all_players if p.replace(
                " ", "").replace(" ", "") not in exclude_list_no_space]

            if not all_players:
                st.info("対象となる選手（背番号登録あり）が見つかりません。")
            else:
                df_period = df_all[df_all["選手名"].isin(all_players)].copy()
                df_last_act = df_period.groupby(
                    "選手名")["Date_all"].max().reset_index()
                df_last_off = df_period[df_period["試合種別"].isin(OFFICIAL_GAME_TYPES)].groupby("選手名")[
                    "Date_all"].max().reset_index()
                ...
                df_alert = df_res[(df_res["days_since_off"] >= 365) | (
                    df_res["最終公式戦日"].isna())].copy()

                if not df_alert.empty:
                    # 表示用データの整形
                    df_alert["最終公式戦日_str"] = df_alert["最終公式戦日"].dt.strftime(
                        '%Y/%m/%d').fillna("公式戦出場なし")
                    df_alert["最終活動日_str"] = df_alert["最終活動日"].dt.strftime(
                        '%Y/%m/%d').fillna("活動記録なし")

                    df_alert["days_since_off"] = (
                        today - df_alert["最終公式戦日"]).dt.days
                    df_alert["days_since_act"] = (
                        today - df_alert["最終活動日"]).dt.days
                    ...
                    def format_days_span(days):
                        ...
                        months = (days % 365) // 30
                        if years > 0:
                            return f"{int(years)}年 {int(months)}ヶ月"
                        elif months > 0:
                            return f"{int(months)}ヶ月"
                        else:
                            return f"{int(days)}日"

                    df_alert["公式戦未参加期間"] = df_alert["days_since_off"].apply(
                        format_days_span)
                    df_alert["全活動未参加期間"] = df_alert["days_since_act"].apply(
                        format_days_span)

                    # 直近1年の参加試合数カウント
                    one_year_ago = today - pd.Timedelta(days=365)
                    df_last_year = df_all[df_all["Date_all"] >= one_year_ago]
                    participation_counts = df_last_year.groupby(
                        "選手名")["Date_all"].nunique().reset_index(name="直近1年参加数")

                    df_alert = pd.merge(
                        df_alert, participation_counts, on="選手名", how="left")
                    df_alert["直近1年参加数"] = df_alert["直近1年参加数"].fillna(
                        0).astype(int)

                    # ソート
                    df_alert = df_alert.sort_values(
                        by="days_since_act", ascending=False)

                    # 警告表示とデータフレーム出力
                    st.error(f"🚨 見直し候補選手が {len(df_alert)} 名います。")

                    display_cols = ["選手名", "最終公式戦日_str", "公式戦未参加期間",
                                    "最終活動日_str", "全活動未参加期間", "直近1年参加数"]
                    display_names = {
                        "最終公式戦日_str": "最後の公式戦",
                        "公式戦未参加期間": "公式戦不参加期間",
                        "最終活動日_str": "最後の活動日",
                    ...
                    st.success("🎉 1年以上公式戦に参加していない対象選手はいません。")
        else:
