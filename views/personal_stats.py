...
def show_personal_stats(df_batting, df_pitching):
    ...
    with t_total:
        ...
        if target_year == "直近2年分":
            ...
            df_b_tg = df_b_tg[df_b_tg["Year"].isin(recent_2years)]
            df_p_tg = df_p_tg[df_p_tg["Year"].isin(recent_2years)]
        elif target_year != "通算":
        ...
        if not df_p_tg.empty:
            ...
            def p_score(r):
                ...
                if p_inn <= 0 or p_era >= 90: return 0.0
        ...
        if not df_all_logs.empty:
            for col in ["日付", "試合種別", "選手名"]:
                if col in df_all_logs.columns: df_all_logs[col] = df_all_logs[col].fillna("").astype(str).str.strip()
        ...
        if all_plist:
            ...
            if not saber_b.empty: m_saber = m_saber.merge(saber_b, on="選手名", how="left")
            if not saber_p.empty: m_saber = m_saber.merge(saber_p, on="選手名", how="left")
            if not saber_f.empty: m_saber = m_saber.merge(saber_f, on="選手名", how="left")
            if not saber_g.empty: m_saber = m_saber.merge(saber_g, on="選手名", how="left")

            fill_cols = ["is_ab", "is_hit", "is_hr", "is_bb", "打点", "盗塁", "盗塁死", "RC", "OPS", "打率", 
                         "投球回", "is_win", "TotalSO", "守備機会", "失策数", "捕手守備機会", "試合参加数",
                         "Batting_Score", "Pitching_Score", "Defense_Score", "Game_Score"]
            for c in fill_cols:
                if c in m_saber.columns: m_saber[c] = m_saber[c].fillna(0)
            if "守備率" in m_saber.columns: m_saber["守備率"] = m_saber["守備率"].fillna(1.0)
            if "防御率" in m_saber.columns: m_saber["防御率"] = m_saber["防御率"].fillna(99.0)

            has_def_data = m_saber["守備機会"].sum() > 0 if "守備機会" in m_saber.columns else False
            m_saber["MVP_Score"] = m_saber["Batting_Score"] + m_saber["Pitching_Score"] + (m_saber["Defense_Score"] if has_def_data else 0) + m_saber["Game_Score"]
        else:
        ...
        with st_mvp:
            if not m_saber.empty:
                ...
                if has_def_data: disp_mvp["守備P"] = disp_mvp["守備P"].map(lambda x: f"{x:.1f}")
        ...
        with st_game:
            if not df_batting.empty or not df_pitching.empty:
                df_b_target = df_batting[df_batting["Year"].isin(years[:2])] if target_year == "直近2年分" else (df_batting[df_batting["Year"] == target_year] if target_year != "通算" else df_batting)
                df_p_target = df_pitching[df_pitching["Year"].isin(years[:2])] if target_year == "直近2年分" else (df_pitching[df_pitching["Year"] == target_year] if target_year != "通算" else df_pitching)
                    ...
                    if not df_personal_logs.empty:
                        ...
                        if not game_stats.empty:
                            ...
                            def format_inactive_period(last_d):
                                if pd.isna(last_d) or last_d == 0: return "-"
                                days = (ref_date - last_d).days
                                if days <= 0: return "直近参加"
                                elif days < 30: return f"{days}日"
                                elif days < 365: return f"{days // 30}ヶ月 ({days}日)"
                                else: return f"{days // 365}年{(days % 365) // 30}ヶ月 ({days}日)"
    ...
    with t_rank_rec:
        ...
        with sub_tab_rec:
            ...
            if not df_bat_res.empty:
                ...
                with tc1: show_top10("打率", df_bat_rate_target, "AVG", "Display", "AVG", suffix="", format_float=True)
                with tc2: show_top10("OPS", df_bat_rate_target, "OPS", "Display", "OPS", suffix="", format_float=True)
                with tc3: show_top10("安打数", df_bat_res, "is_hit", "Display", "is_hit", suffix=" 本")
                
                st.write("")
                tc4, tc5, tc6 = st.columns(3)
                with tc4: show_top10("本塁打", df_bat_res, "is_hr", "Display", "is_hr", suffix=" 本")
                with tc5: show_top10("打点", df_bat_res, "打点", "Display", "打点", suffix=" 点")
                with tc6: show_top10("盗塁", df_bat_res, "盗塁", "Display", "盗塁", suffix=" 個")
            else:
                st.info("打撃データがありません")
            ...
            if not df_pit_res.empty:
                ...
                with tp1: show_top10("防御率", df_pit_rate_target, "ERA", "Display", "ERA", ascending=True, format_float=True)
                with tp2: show_top10("WHIP", df_pit_rate_target, "WHIP", "Display", "WHIP", ascending=True, format_float=True)
                with tp3: show_top10("勝利数", df_pit_res, "is_win", "Display", "is_win", suffix=" 勝")
                with tp4: show_top10("奪三振", df_pit_res, "TotalSO", "Display", "TotalSO", suffix=" 個")
            else:
                st.info("投手データがありません")
        ...
        with sub_tab_period:
            ...
            if not df_b_sub.empty:
                ...
                with r1: show_top10("打率", rank_b[rank_b["Total_PA"]>=min_ab], "AVG", "選手名", "AVG", format_float=True)
                with r2: show_top10("本塁打", rank_b, "is_hr", "選手名", "is_hr", suffix="本")
                with r3: show_top10("打点", rank_b, "打点", "選手名", "打点", suffix="点")
                
                st.write("")
                r4, r5, r6 = st.columns(3)
                with r4: show_top10("安打", rank_b, "is_hit", "選手名", "is_hit", suffix="本")
                with r5: show_top10("盗塁", rank_b, "盗塁", "選手名", "盗塁", suffix="個")
                with r6: show_top10("OPS", rank_b[rank_b["is_ab"]>=min_ab], "OPS", "選手名", "OPS", format_float=True)
            else: st.info("データなし")
            st.divider()
            ...
            if not df_p_sub.empty:
                ...
                with p1: show_top10("防御率", rank_p[rank_p["Innings"]>=min_inn], "ERA", "選手名", "ERA", ascending=True, format_float=True)
                with p2: show_top10("WHIP", rank_p[rank_p["Innings"]>=min_inn], "WHIP", "選手名", "WHIP", ascending=True, format_float=True)
                with p3: show_top10("勝利", rank_p, "is_win", "選手名", "is_win", suffix="勝")
                with p4: show_top10("奪三振", rank_p, "TotalSO", "選手名", "TotalSO", suffix="個")
            else: st.info("データなし")