...
def show_personal_stats(df_batting, df_pitching):
    ...
    # 集計ルール
    agg_rules_b = {"is_hit": "sum", "is_ab": "sum", "is_hr": "sum", "打点": "sum", "盗塁": "sum", "得点": "sum", "is_bb": "sum", "bases": "sum"}
    ...
    with t_total:
        ...
        with st_bat:
            if not df_b_tg.empty:
                ...
                for c in ["is_hit", "is_ab", "is_hr", "打点", "盗塁"]: stats[c] = stats[c].astype(int)

                disp = stats.rename(columns={"is_hit":"安打", "is_ab":"打数", "is_hr":"本塁打"}).sort_values("打率", ascending=False)
                disp["打率"] = disp["打率"].map(lambda x: f"{x:.3f}")
                st.dataframe(disp[["選手名", "打率", "打数", "安打", "本塁打", "打点", "盗塁"]], use_container_width=True, hide_index=True)
            else: st.info("データなし")

        with st_pit:
            if not df_p_tg.empty:
                stats_p = df_p_tg.groupby("投手名").agg(agg_rules_p).reset_index()
                ...
                disp_p = stats_p[["投手名", "防御率", "is_win", "is_lose", "投球回", "TotalSO", "自責点"]].copy()
                disp_p.columns = ["投手名", "防御率", "勝", "敗", "投球回", "奪三振", "自責点"]
        ...
        if sel_player:
            if not df_b_calc.empty:
                ...
                if not my_b.empty:
                    ...
                    for col in ["is_hit", "is_ab", "is_hr", "打点", "盗塁"]: hist[col] = hist[col].astype(int)
                    ...
                    st.dataframe(disp_hist.style.format({"打率": "{:.3f}"}))
                else: st.info("データなし")
            
            if not df_p_calc.empty:
                my_p = df_p_calc[df_p_calc["投手名"] == sel_player]
                if not my_p.empty:
    ...
    with t_rank:
        ...
        if not df_p_sub.empty:
            rank_p = get_ranking_df(df_p_sub, ["投手名"], agg_rules_p)
            ...
            with p1: show_top5("防御率", rank_p[rank_p["Innings"]>=min_inn], "ERA", "投手名", "ERA", ascending=True, format_float=True)
            with p2: show_top5("WHIP", rank_p[rank_p["Innings"]>=min_inn], "WHIP", "投手名", "WHIP", ascending=True, format_float=True)
            with p3: show_top5("勝利", rank_p, "is_win", "投手名", "is_win", suffix="勝")
            with p4: show_top5("奪三振", rank_p, "TotalSO", "投手名", "TotalSO", suffix="個")
        else: st.info("データなし")
