...
def show_personal_stats(df_batting, df_pitching):
    ...
    with t_total:
        ...
        if not df_b_tg.empty:
            df_b_tg = df_b_tg[~df_b_tg["選手名"].str.strip().isin([name.strip() for name in HIDDEN_PLAYERS_TOTAL])]
        
        if not df_p_tg.empty:
            df_p_tg = df_p_tg[~df_p_tg["選手名"].str.strip().isin([name.strip() for name in HIDDEN_PLAYERS_TOTAL])]
