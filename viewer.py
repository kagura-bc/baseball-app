...
def inning_sort_key(inn_str):
    ...
    if "スタメン" in s:
        return 0
    try:
        return int(s.replace("回", ""))
    except:
        return 99 

# render_scoreboard 関数は変更なしでOK
...
if menu == "🏠 最新試合結果":
    ...
    if not df_batting.empty or not df_pitching.empty:
        ...
        if not df_pitching.empty:
            df_pitching["Date"] = pd.to_datetime(df_pitching["日付"], errors='coerce')
            p_max = df_pitching["Date"].max()
            if pd.notna(p_max):
                if latest_date is None or p_max > latest_date:
                    latest_date = p_max
        ...
        if pd.notna(latest_date):
            ...
            # カラム定義（エラー回避用）
            cols_b = ["日付", "イニング", "選手名", "位置", "結果", "打点", "得点", "盗塁", "種別", "グラウンド", "対戦相手", "試合種別"]
            cols_p = ["日付", "イニング", "投手名", "結果", "処理野手", "球数", "アウト数", "失点", "自責点", "グラウンド", "対戦相手", "試合種別", "勝敗"]

            # 2. その日のデータを抽出
            if not df_batting.empty:
                day_b = df_batting[df_batting["Date"] == latest_date]
            else:
                day_b = pd.DataFrame(columns=cols_b)

            if not df_pitching.empty:
                day_p = df_pitching[df_pitching["Date"] == latest_date]
            else:
                day_p = pd.DataFrame(columns=cols_p)
            ...
            elif not day_p.empty:
                opp = day_p["対戦相手"].iloc[0] if "対戦相手" in day_p.columns else "-"
                ground = day_p["グラウンド"].iloc[0] if "グラウンド" in day_p.columns else "-"
                m_type = day_p["試合種別"].iloc[0] if "試合種別" in day_p.columns else "-"
            else:
                opp, ground, m_type = "-", "-", "-"
            ...
            # --- 打撃詳細（viewer.py） ---
            # --- 🏠 最新試合結果 の with tab_d_bat: をこれに書き換える ---
            with tab_d_bat:
                if not day_b.empty:
                    # イニング列を強制的に文字列にし、全角半角スペースを除去して判定
                    day_b["check_inn"] = day_b["イニング"].astype(str).str.replace(" ", "").str.replace("　", "")
                    
                    # "ベンチ" を含む行を特定
                    is_bench = day_b["check_inn"].str.contains("ベンチ", na=False)
                    
                    game_rows = day_b[~is_bench].copy()
                    bench_rows = day_b[is_bench].copy()
                    
                    # --- A. 出場選手 ---
                    if not game_rows.empty:
                        # イニング順・打順順にソート
                        def safe_inning_sort(x):
                            try:
                                return int(str(x).replace("回", ""))
                            except:
                                return 99
                        
                        game_rows["_inn_sort"] = game_rows["イニング"].apply(safe_inning_sort)
                        game_rows_sorted = game_rows.sort_values(by=["_inn_sort", "日付"], kind='mergesort')
                        
                        players_ordered = game_rows_sorted["選手名"].unique()
                        played_players = set(players_ordered) # セット作成
                        
                        batting_rows = []
                        max_at_bats = 0 
                        display_targets = ["単打", "二塁打", "三塁打", "本塁打", "三振", "四球", "死球", "犠打", "凡退", "失策", "打撃妨害", "併殺打", "走塁死", "犠飛"]

                        for i, p_name in enumerate(players_ordered):
                            p_rows = game_rows_sorted[game_rows_sorted["選手名"] == p_name]
                        ...
                        if batting_rows:
                            ...
                            final_max_cols = max(3, max_at_bats)
                        ...
                        st.info("出場選手データがまだありません")
                        played_players = set()

                    # --- B. ベンチ入りメンバー ---
                    if not bench_rows.empty:
                        # ベンチ登録された名前のユニークリスト
                        bench_names = bench_rows["選手名"].unique()
                        # 試合に出場していない選手だけを抽出
                        final_bench_list = [p for p in bench_names if p not in played_players]
                        
                        if final_bench_list:
                            st.divider()
                            st.markdown("##### 🛋️ ベンチ入りメンバー")
                            
                            cols = st.columns(6)
                            for idx, p_name in enumerate(final_bench_list):
                                p_num = PLAYER_NUMBERS.get(p_name, "")
                                disp_name = f"{p_name} (#{p_num})" if p_num else p_name
                                cols[idx % 6].info(disp_name)
                    else:
                        # ベンチデータが1件もない場合
                        pass
                else:
...
# --- 👑 歴代記録 (修正版) ---
    ...
    if df_batting.empty or df_pitching.empty:
        ...
        # ---------------------------------------------------------
        # 1. データ前処理：年度(Year)と数値型の整理
        # ---------------------------------------------------------
        df_b_calc = df_batting.copy()
        df_p_calc = df_pitching.copy()

        # 日付変換 & エラー行除去
        df_b_calc["temp_date"] = pd.to_datetime(df_b_calc["日付"], errors='coerce')
        df_p_calc["temp_date"] = pd.to_datetime(df_p_calc["日付"], errors='coerce')

        # 年度の抽出（2025.0 -> 2025）
        df_b_calc["Year"] = df_b_calc["temp_date"].dt.year.fillna(0).astype(int).astype(str)
        df_p_calc["Year"] = df_p_calc["temp_date"].dt.year.fillna(0).astype(int).astype(str)

        # "0"年（日付不明）を除外
        df_b_calc = df_b_calc[df_b_calc["Year"] != "0"]
        df_p_calc = df_p_calc[df_p_calc["Year"] != "0"]

        # --- A. 打撃データの計算用列作成 ---
        hit_cols = ["単打", "二塁打", "三塁打", "本塁打"]
        ab_cols = hit_cols + ["凡退", "失策", "走塁死", "併殺打", "三振"] # 打数に含まれるもの
        ...
        if "勝敗" in df_p_calc.columns:
            df_p_calc["is_win"] = df_p_calc["勝敗"].astype(str).str.contains("勝", na=False).astype(int)
        else:
            df_p_calc["is_win"] = 0
        ...
        # 3. 表示用関数（ランキング生成）
        # ---------------------------------------------------------
        def show_top5(title, df, sort_col, label_col, value_col, ascending=False, suffix="", format_float=False, float_fmt="{:.3f}"):
            st.markdown(f"##### {title}")
        ...
        with tab_season:
            ...
            with c1:
                if not season_bat.empty:
                    f_avg = season_bat[season_bat["is_ab"] >= min_ab]
                    show_top5("打率", f_avg, "AVG", "Display", "AVG", format_float=True)
            ...
            with p1:
                if not season_pit.empty:
                    f_era = season_pit[season_pit["Innings"] >= min_inn]
                    show_top5("防御率", f_era, "ERA", "Display", "ERA", ascending=True, format_float=True, float_fmt="{:.2f}")
                else: st.caption("データなし")
            with p2:
        ...
        with tab_career:
            ...
            with tc1:
                if not career_bat.empty:
                    # 通算は基準を高めに（例: 20打数以上）
                    min_ab_career = min_ab * 2
                    f_c_avg = career_bat[career_bat["is_ab"] >= min_ab_career]
                    show_top5("通算打率", f_c_avg, "AVG", "Display", "AVG", format_float=True)
                else: st.caption("データなし")
            with tc2:
            ...
            with tp1:
                if not career_pit.empty:
                    min_inn_career = min_inn * 2
                    f_c_era = career_pit[career_pit["Innings"] >= min_inn_career]
                    show_top5("通算防御率", f_c_era, "ERA", "Display", "ERA", ascending=True, format_float=True, float_fmt="{:.2f}")
                else: st.caption("データなし")
            with tp2:
