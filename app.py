...
def load_pitching_data():
    try:
        # ▼▼▼ 修正: ttl=0 だと毎回通信してしまうため、"10m" (10分) に変更 ▼▼▼
        # 更新処理の直後には st.cache_data.clear() をしているため、
        # ここを 10m にしても最新データの反映には影響しません。
    ...
    # ---------------------------------------------------------
        ...
        def submit_batting():
            ...
            for i in range(15):
                ...
                p_res = st.session_state.get(f"sr{i}", "---") # 結果を取得

                # ▼▼▼ 修正: 名前があり、かつ結果が「---」以外のときだけ登録する ▼▼▼
                if p_name and p_res != "---":
        ...
        # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
                ...
                # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
                # 【スタメン一括登録ボタン】
                # これを押すことで、打席結果がなくてもビューワーに選手・守備・打順が表示されます
                # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
                if st.button("スタメン登録 (試合開始)", key="start_game_btn", help="現在のオーダーを登録します。試合開始時に押すとビューワーに反映されます。"):
                    starter_records = []
                    for i in range(15):
                        p_name = st.session_state.get(f"sn{i}")
                        p_pos = st.session_state.get(f"sp{i}", "")
                        
                        if p_name: # 名前が入っていれば登録
                            starter_records.append({
                                "日付": selected_date_str, 
                                "グラウンド": ground_name,
                                "対戦相手": opp_team, 
                                "試合種別": match_type,
                                "イニング": "試合開始",  # スタメン用の特別なイニング名
                                "選手名": p_name, 
                                "位置": p_pos,        # 守備位置を保存
                                "打順": i + 1,        # 打順を保存
                                "結果": "スタメン",    # 集計に影響しない結果名
                                "打点": 0, "得点": 0, "盗塁": 0, "種別": "スタメン"
                            })
                    
                    if starter_records:
                        try:
                            # データ結合して保存
                            updated_df = pd.concat([df_batting, pd.DataFrame(starter_records)], ignore_index=True)
                            conn.update(spreadsheet=SPREADSHEET_URL, data=updated_df)
                            st.cache_data.clear()
                            st.success(f"✅ スタメン {len(starter_records)} 名を登録しました！ビューワーを確認してください。")
                            import time
                            time.sleep(1.0)
                            st.rerun()
                        except Exception as e:
                        ...
                        st.warning("選手が入力されていません。")

                # --- UIレイアウト (既存の登録ボタン) ---
                st.button("登録実行 (1打席)", type="primary", on_click=submit_batting, use_container_width=True)
            ...
            if not today_batting_df.empty:
                ...
                if current_name_val:
                    ...
                    if not p_df.empty:
                        ...
                        # ▼▼▼ 修正: 打席数をカウントするための変数と定義 ▼▼▼
                        pa_count = 0
                        # 打席としてカウントする結果のリスト
                        pa_list = ["単打", "二塁打", "三塁打", "本塁打", "三振", "四球", "死球", "犠打", "凡退", "失策", "併殺打", "野選", "振り逃げ", "打撃妨害"]

                        for _, row in p_df.iterrows(): # reset_indexは使わず手動カウント
                            res_text = row['結果']
                            original_res = res_text # 判定用に元の値を保持
                            ...
                            # ▼▼▼ 打撃結果なら打席数を付与、それ以外（盗塁など）はカッコのみ ▼▼▼
                            if original_res in pa_list:
                                pa_count += 1
                                history_items.append(f"{pa_count}打席({res_text})")
                            else:
                                # 盗塁死、牽制死、得点、盗塁などは打席数をつけない
                                history_items.append(f"({res_text})")
...
# ---------------------------------------------------------
    ...
    if input_mode_p == "詳細入力 (1打席ごと)":
        ...
        # =========================================================
        #  【修正版】結果入力フォーム (誤登録防止・デフォルト空欄版)
        # =========================================================
        
        # 0. 現在のスタメン守備陣を取得
        current_defense_map = {}
        for i in range(15):
            pos_key = f"sp{i}"; name_key = f"sn{i}"
            pos = st.session_state.get(pos_key) or st.session_state.get("saved_lineup", {}).get(f"pos_{i}")
            name = st.session_state.get(name_key) or st.session_state.get("saved_lineup", {}).get(f"name_{i}")
            if pos and name and name in all_players:
                current_defense_map[pos] = name

            # ▼▼▼▼▼▼▼▼▼▼▼▼▼ 追加コードここから ▼▼▼▼▼▼▼▼▼▼▼▼▼
        
        # 1. 解説用モーダルの定義（関数内関数として定義）
        @st.dialog("自責点 (Earned Run) の判定ガイド")
        def show_er_guide():
            st.markdown("""
            ### ⚾ 自責点とは？
            **「投手が責任を持つ失点」**のことです。
            ...
            #### ✅ 自責点になるケース
            * **安打、四死球、犠打、犠飛、野選、盗塁** による失点。
            * 暴投（ワイルドピッチ）やボークによる進塁・失点。
            
            #### ❌ 自責点にならないケース (0点)
            * **味方の失策（エラー）** や **捕逸（パスボール）** がなければ防げたはずの失点。
            * エラーで出塁した走者がホームインした場合。
            
            ---
            
            #### ⚠️ 重要な判断基準 (2アウト後の特例)
            **「もしエラーがなければ、すでに3アウトでチェンジになっていた」** と判断される場合、
            その後の失点は**すべて自責点0**になります。
            
            > 例：2アウト走者なし。次打者がショートゴロエラーで出塁（本来ならここでチェンジ）。
            > その後、ホームランを打たれて2点入った。
            > ➝ **自責点は0** です。（本来ならイニングが終わっていたため）
            """)

        # 2. 解説ボタンの配置 (フォームの外に置くのがポイント)
        # ※これを押してもフォームの内容は消えませんが、スクリプトは再実行されます
        ...
        with st.form("pitching_detail_form_outs"):
            ...
            with c_res:
                p_res = st.selectbox(
                    "打席結果",
                    ["凡退", "三振", "安打", "本塁打", "四球", "死球", "犠打", "失策", "併殺打", "野選"],
                    key="p_det_res"
                )

            # ▼▼▼ 【修正】デフォルトを空欄にする ▼▼▼
            target_fielder_pos = ""
            needs_fielder = ["凡退", "失策", "併殺打", "犠打", "野選"]
            
            with c_fld:
                if p_res in needs_fielder:
                    # 先頭に空文字を追加して、デフォルトを空欄にする
                    positions = [""] + ["投", "捕", "一", "二", "三", "遊", "左", "中", "右"]
                    target_fielder_pos = st.selectbox("打球方向 / 処理野手", positions, key="p_det_fielder")
                    
                    if target_fielder_pos:
                        if target_fielder_pos in current_defense_map:
                            st.caption(f"守備: {current_defense_map[target_fielder_pos]}")
                        else:
                            st.caption("※選手未登録")
                    else:
                        st.write("") # 未選択時は表示なし
            ...
            with c_run:
                p_runs = st.number_input("失点", min_value=0, step=1, key="p_det_r")
            with c_er:
                p_er   = st.number_input("自責", min_value=0, step=1, key="p_det_er")
            ...
            if submit_detail:
                ...
                # ▼▼▼ 【追加】守備位置の未選択チェック ▼▼▼
                elif p_res in needs_fielder and target_fielder_pos == "":
                    st.error("⚠️ 「打球方向 / 処理野手」を選択してください。")
                    ...
                    try:
                        ...
                        # アウト数・被安打計算
                        add_outs = 0
                        if p_res in ["三振", "凡退", "犠打", "犠飛", "野選"]: add_outs = 1
                        ...
                        saved_fielder_str = ""
                        if target_fielder_pos:
                            f_name = current_defense_map.get(target_fielder_pos, "不明")
                            saved_fielder_str = f"{f_name} ({target_fielder_pos})"
                        ...
                        if "df_pitching" in st.session_state: del st.session_state["df_pitching"]
                        st.cache_data.clear()
                        ...
                        # メッセージ・画面更新
                        msg = f"  ✅   {p_res}"
                        if saved_fielder_str: msg += f" (処理: {saved_fielder_str})"
        ...
        if not today_pitching_df.empty:
            # ▼▼▼ 1. プルダウン（セレクトボックス）の作成 ▼▼▼
            
            # データが存在するイニングのリストを取得してソート
            unique_inns = list(today_pitching_df["イニング"].unique())
            # "1回", "2回"... の順に並ぶようにソート（数値変換して比較）
            try:
                unique_inns.sort(key=lambda x: int(str(x).replace("回", "")) if "回" in str(x) else 99)
            except:
                pass # ソート失敗時はそのまま

            # 選択肢：「全イニング」を先頭に追加
            filter_options = ["全イニング"] + unique_inns

            # 左側にプルダウン、右側は空白
            c_filter_mat, _ = st.columns([2, 3])
            with c_filter_mat:
                target_inn_filter = st.selectbox(
                    "表示イニングを選択", 
                    filter_options, 
                    key="matrix_filter_select"
                )

            # ▼▼▼ 2. データのフィルタリング処理 ▼▼▼
            target_matrix_df = today_pitching_df.copy()
            
            # 「全イニング」以外が選ばれている場合、そのイニングで絞り込み
            if target_inn_filter != "全イニング":
                 target_matrix_df = target_matrix_df[target_matrix_df["イニング"] == target_inn_filter]

            # --- 以下、表の生成ロジック（データソースを target_matrix_df に変更） ---
            matrix_data = []
            max_at_bats = 0 
            
            for i in range(1, st.session_state["opp_batter_count"] + 1):
                target_str = f"{i}番打者"
                # フィルタ済みの target_matrix_df を使用
                batter_df = target_matrix_df[target_matrix_df["種別"].astype(str).str.contains(target_str, na=False)]
            ...
            if matrix_data and max_at_bats > 0:
                ...
                # データがない場合の表示
                if target_inn_filter != "全イニング":
                    st.caption(f"※ {target_inn_filter}のデータはありません")
                else:
...
# --------------------------------------------------
            ...
            if selected_label:
                ...
                if target_date_str:
                    ...
                    if not personal_bat.empty:
                        ...
                        if not df_active.empty:
                            def summarize_batting_text(df_group):
                                ...
                                summary_str = " ".join(res_parts) if res_parts else "" 
                            ...
                            # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
                            # 【修正】打順の自動補完ロジック (救済措置)
                            # データに打順が入っていない(999)場合、入力順(上から順)に連番を振ります。
                            # これにより、過去のデータやスタメン登録忘れでも打順が表示されます。
                            # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
                            # 暫定的にリストを作成
                            temp_orders = []
                            for i in range(len(df_summary)):
                                ...
                                if current_val == 999:
                                    # データがない場合は連番(1, 2, 3...)を割り当て
                                    temp_orders.append(i + 1)
                                else:
                                    # データがある場合はそれを使う
                                    temp_orders.append(current_val)
                    ...
                    if not personal_pit.empty:
                        ...
                        # ▼▼▼ 修正: 名前解決のロジック強化 ▼▼▼
                        # -------------------------------------------------------
                        # 1. 「選手名」があれば、それを「投手名」の空欄箇所に優先的に埋める
                        if "選手名" in personal_pit.columns:
                            if "投手名" not in personal_pit.columns:
                                personal_pit["投手名"] = personal_pit["選手名"]
                            else:
                                # 既にカラムがある場合でも、空文字("")なら選手名を採用する
                                personal_pit["投手名"] = personal_pit["投手名"].replace("", pd.NA).fillna(personal_pit["選手名"])
                        ...
                        # 集計用関数の定義
                        def summarize_pitching_series(df_group):
                            ...
                            for _, row in df_group.iterrows():
                                raw_h = row.get("被安打", 0)
                                val_h = int(raw_h) if pd.notna(raw_h) else 0
                                raw_so = row.get("奪三振", 0)
                                val_so = int(raw_so) if pd.notna(raw_so) else 0
                                raw_bb = row.get("与四球", 0) 
                                val_bb = int(raw_bb) if pd.notna(raw_bb) else 0
                                res = str(row.get("結果", ""))
                        ...
                        # リストを作成して集計
                        summary_list = []
                        for p_name, group in personal_pit.groupby("投手名", sort=False, dropna=False):
                            data_dict = summarize_pitching_series(group)
                            summary_list.append(data_dict)
                        
                        # リストからDataFrameを作成
                        df_p_summary = pd.DataFrame(summary_list)
                        
                        # 表を表示
                        if not df_p_summary.empty:
                            st.table(df_p_summary[["投手名", "結果", "回", "球数", "被安", "奪三", "四死", "失点", "自責"]].set_index("投手名"))
                        else:
                            st.warning("集計結果が0件でした。")
        ...
        with t_pit:
            if not df_p_target.empty:
                ...
                for p in all_players:
                    ...
                    # ▼▼▼ 集計ロジック修正 (詳細入力 + まとめ入力) ▼▼▼
                    # 1. 奪三振 (詳細の行数 + まとめ入力の"奪三振"列)
                    k_detailed = pd_p[(pd_p["イニング"] != "まとめ入力") & (pd_p["結果"] == "三振")].shape[0]
                    k_bulk = pd_p[pd_p["イニング"] == "まとめ入力"]["奪三振"].sum() if "奪三振" in pd_p.columns else 0
                    ks = k_detailed + k_bulk

                    # 2. 四死球 (詳細の行数 + まとめ入力の"与四球"列)
                    bb_detailed = pd_p[(pd_p["イニング"] != "まとめ入力") & (pd_p["結果"].isin(["四球", "死球"]))].shape[0]
                    bb_bulk = pd_p[pd_p["イニング"] == "まとめ入力"]["与四球"].sum() if "与四球" in pd_p.columns else 0
                    bbs = bb_detailed + bb_bulk

                    # 3. 被安打 (詳細・まとめ共に"被安打"カラムに数値が入る仕様のため合計する)
                    # ※詳細入力時も save_lineup_item 等で被安打=1 が記録されている前提
                    total_hits = pd_p["被安打"].sum()

                    # 4. WHIP ( (与四球 + 被安打) / 投球回 )
                    # ※ここでのbbsは死球を含んでいる可能性がありますが、簡易的に (BB+H)/IP とします
                    whip = (bbs + total_hits) / innings if innings > 0 else 0.00
                    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
                ...
                if pitch_stats:
                    ...
                    # 小数点フォーマット
                    for col in ["防御率", "WHIP", "奪三振率", "勝率"]:
                        df_p_res[col] = df_p_res[col].map(lambda x: f"{x:.2f}" if col != "勝率" else f"{x:.3f}")
                    
                    # カラムの並び順整理 (WHIPを追加)
                    cols_order = ["No.", "氏名", "防御率", "WHIP", "勝利", "敗戦", "勝率", "投球回", "奪三振", "奪三振率", "四死球", "失点"]
    ...
    with t_fld:
        st.markdown("#####   🛡️   守備成績")
        st.caption("※ 凡退・併殺打・犠打などの処理数（刺殺・補殺）と、失策数を集計します")

        if not df_p_target.empty and "処理野手" in df_p_target.columns:
            # 処理野手が記録されているデータだけ抽出
            fld_data = df_p_target[df_p_target["処理野手"] != ""].copy()
            ...
            if not fld_data.empty:
                ...
                # 2. 列の整理 (「併殺打」を追加)
                target_cols = ["凡退", "犠打", "失策", "走塁死", "牽制死", "盗塁死", "併殺打"]
                for col in target_cols:
                    if col not in stats.columns:
                        stats[col] = 0

                # 3. 指標計算
                # 失策以外はすべて「アウトに関与した（刺殺・補殺）」としてカウント
                ...
                if len(display_rows) > 0:
                    ...
                    # 表示カラム定義 (併殺打を追加)
                    final_cols = ["位置", "No.", "氏名", "守備機会", "刺殺・補殺", "併殺打", "失策", "守備率"]
        ...
        st.title("👑 チーム歴代記録")
        st.write("過去の全データから、各種ランキングや個人の年度別成績を表示します。")
        ...
        if df_batting.empty or df_pitching.empty:
            ...
            agg_rules_b = {
                ...
                "is_1b": "sum", "is_2b": "sum", "is_3b": "sum", "is_bb": "sum", # 追加
            ...
            # 投手名補完
            if "選手名" in df_p_calc.columns and "投手名" in df_p_calc.columns:
            ...
            # 数値変換 (被安打、与四球を追加)
            for col in ["自責点", "失点", "アウト数", "被安打", "与四球"]:
                if col not in df_p_calc.columns: df_p_calc[col] = 0
                df_p_calc[col] = pd.to_numeric(df_p_calc[col], errors='coerce').fillna(0)
            
            # 四死球の計算（詳細入力の「結果」と、まとめ入力の「与四球」カラムを合算）
            ...
            agg_rules_p = {
                ...
                "被安打": "sum", "total_bb": "sum"  # ★ここに追加
            }

                # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
                # 【移動・追加】 ここに関数を定義します（2180行目付近）
                # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
            ...
            def show_top5(title, df, sort_col, label_col, value_col, ascending=False, suffix="", format_float=False):
                st.markdown(f"##### {title}")
                if ascending:
                    target_df = df.copy() # 防御率などは0.00も含める
                else:
            ...
            # タブ切り替え
            # ==================================================
            tab_season, tab_career, tab_personal = st.tabs(["   📅    シーズン記録 (年度別)", "   🏅    歴代記録 ", "   👤    個人年度別"])

            # --- 1. シーズン記録 (年度選択機能付き + 通算対応) ---
            with tab_season:
                # (1) 年度別データの準備
                season_bat = get_ranking_df(df_b_calc, ["Year", "選手名"], agg_rules_b)
                season_pit = get_ranking_df(df_p_calc, ["Year", "投手名"], agg_rules_p)
                
                # ▼▼▼ ここで関数を呼び出してOPS等を計算します！ ▼▼▼
                season_bat = calc_advanced_stats(season_bat)
                # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

                season_pit["Innings"] = season_pit["アウト数"] / 3
                season_pit["ERA"] = season_pit.apply(lambda x: (x["自責点"] * 7) / x["Innings"] if x["Innings"] > 0 else 99.99, axis=1)
                season_pit["WHIP"] = season_pit.apply(lambda x: (x["total_bb"] + x["被安打"]) / x["Innings"] if x["Innings"] > 0 else 99.99, axis=1)

                season_bat["Display"] = season_bat["選手名"]
                season_pit["Display"] = season_pit["投手名"]

                # (2) 選択肢の作成
                years_b = season_bat["Year"].unique().tolist() if "Year" in season_bat.columns else []
                years_p = season_pit["Year"].unique().tolist() if "Year" in season_pit.columns else []
                unique_years = sorted(list(set(years_b + years_p)), reverse=True)

                if not unique_years:
                    st.info("データがありません。")
                else:
                    # 選択肢リスト: 先頭に「通算」を追加
                    all_options = ["通算"] + unique_years
                    
                    # デフォルト選択: 最新年 (リストの2番目=index 1) に設定
                    # ※データがある場合、all_optionsは ["通算", "2025", "2024"...] となるため index=1 が最新年
                    default_idx = 1 if len(unique_years) > 0 else 0

                    c_year_sel, c_fil1, c_fil2 = st.columns([1, 1, 1])
                    target_year = c_year_sel.selectbox("年度を選択", all_options, index=default_idx, key="season_rank_year")

                    # (3) データの切り替えロジック
                    if target_year == "通算":
                        df_bat_target = get_ranking_df(df_b_calc, ["選手名"], agg_rules_b)
                        df_pit_target = get_ranking_df(df_p_calc, ["投手名"], agg_rules_p)
                        ...
                        # ラベルと初期値設定
                        lbl_ab, val_ab = "通算の規定打席", 30
                        lbl_inn, val_inn = "通算の規定投球回", 15
                        
                        header_suffix = " (通算)"
                    else:
                        # ▼ 年度別データの抽出
                        df_bat_target = season_bat[season_bat["Year"] == target_year].copy()
                        df_pit_target = season_pit[season_pit["Year"] == target_year].copy()
                        
                        # ラベルと初期値設定
                        lbl_ab, val_ab = f"{target_year}年の規定打席", 10
                        lbl_inn, val_inn = f"{target_year}年の規定投球回", 5
                        
                        header_suffix = f" ({target_year}年)"

                    # (4) 規定数の入力欄
                    min_ab = c_fil1.number_input(lbl_ab, value=val_ab, min_value=1)
                    min_inn = c_fil2.number_input(lbl_inn, value=val_inn, min_value=1)

                    # (5) ランキング表示
                    st.markdown(f"####   ⚔️   打撃部門{header_suffix}")
                    
                    # --- 1段目: 率系の指標 (打率, OPS, 出塁率) ---
                    c1, c2, c3 = st.columns(3)
                    with c1:
                        filtered_avg = df_bat_target[df_bat_target["is_ab"] >= min_ab]
                        show_top5("打率", filtered_avg, "AVG", "Display", "AVG", suffix="", format_float=True)
                    with c2: 
                        # OPS (規定打席あり)
                        filtered_ops = df_bat_target[df_bat_target["is_ab"] >= min_ab]
                        show_top5("OPS", filtered_ops, "OPS", "Display", "OPS", suffix="", format_float=True)
                    with c3: 
                        # 出塁率 (規定打席あり)
                        filtered_obp = df_bat_target[df_bat_target["is_ab"] >= min_ab]
                        show_top5("出塁率", filtered_obp, "OBP", "Display", "OBP", suffix="", format_float=True)

                    st.write("") # 行間の余白

                    # --- 2段目: 打撃主要スタッツ (本塁打, 打点, 安打) ---
                    c4, c5, c6 = st.columns(3)
                    with c4: show_top5("本塁打", df_bat_target, "is_hr", "Display", "is_hr", suffix=" 本")
                    with c5: show_top5("打点", df_bat_target, "打点", "Display", "打点", suffix=" 点")
                    with c6: show_top5("安打数", df_bat_target, "is_hit", "Display", "is_hit", suffix=" 本")
                    
                    st.write("") # 行間の余白

                    # --- 3段目: 走塁・得点系 (盗塁, 得点, 空白) ---
                    c7, c8, c9 = st.columns(3)
                    with c7: show_top5("盗塁", df_bat_target, "盗塁", "Display", "盗塁", suffix=" 個")
                    with c8: show_top5("得点", df_bat_target, "得点", "Display", "得点", suffix=" 点")
                    with c9: st.write("") # 空白（または四死球などを入れてもOK）

                    st.divider()
                    st.markdown(f"####   🛡️   投手部門{header_suffix}")
                    p1, p2, p3, p4 = st.columns(4)
                    with p1:
                        filtered_era = df_pit_target[df_pit_target["Innings"] >= min_inn]
                        show_top5("防御率", filtered_era, "ERA", "Display", "ERA", ascending=True, suffix="", format_float=True)
                    with p2:
                        # ▼ WHIPを追加
                        filtered_whip = df_pit_target[df_pit_target["Innings"] >= min_inn]
                        show_top5("WHIP", filtered_whip, "WHIP", "Display", "WHIP", ascending=True, suffix="", format_float=True)
                    with p3: show_top5("勝利数", df_pit_target, "is_win", "Display", "is_win", suffix=" 勝")
                    with p4: show_top5("奪三振", df_pit_target, "is_so", "Display", "is_so", suffix=" 個")
            ...
            with tab_career:
                ...
                rec_type = st.radio(
                    "集計対象を選択", 
                    ["歴代シーズン最高 (Season High)", "生涯通算 (Lifetime Total)"], 
                    horizontal=True
                )
                
                # データ準備
                if "Season" in rec_type:
                    # A. 歴代シーズン最高
                    df_bat_target = get_ranking_df(df_b_calc, ["Year", "選手名"], agg_rules_b)
                    df_pit_target = get_ranking_df(df_p_calc, ["Year", "投手名"], agg_rules_p)
                    
                    # ▼▼▼ 計算適用 ▼▼▼
                    df_bat_target = calc_advanced_stats(df_bat_target)
                    # ▲▲▲▲▲▲▲▲▲▲▲▲
                    
                    df_bat_target["Display"] = df_bat_target["選手名"] + " (" + df_bat_target["Year"] + "年)"
                    df_pit_target = get_ranking_df(df_p_calc, ["Year", "投手名"], agg_rules_p)
                    ...
                    # ★ここで年度を表示に追加
                    df_bat_target["Display"] = df_bat_target["選手名"] + " (" + df_bat_target["Year"] + "年)"
                    df_pit_target["Display"] = df_pit_target["投手名"] + " (" + df_pit_target["Year"] + "年)"
                ...
                with tc1:
                    # 打率など率系は規定打席フィルタ
                    filtered_c_avg = df_bat_target[df_bat_target["is_ab"] >= min_ab]
                    show_top5("打率", filtered_c_avg, "AVG", "Display", "AVG", suffix="", format_float=True)
                with tc2:
                    # OPS
                    filtered_c_ops = df_bat_target[df_bat_target["is_ab"] >= min_ab]
                    show_top5("OPS", filtered_c_ops, "OPS", "Display", "OPS", suffix="", format_float=True)
                with tc3:
                    # 出塁率
                    filtered_c_obp = df_bat_target[df_bat_target["is_ab"] >= min_ab]
                    show_top5("出塁率", filtered_c_obp, "OBP", "Display", "OBP", suffix="", format_float=True)

                st.write("") # 行間

                # 2段目: 積み上げ系指標 (本塁打, 打点, 安打)
                ...
                with tc6: show_top5("安打数", df_bat_target, "is_hit", "Display", "is_hit", suffix=" 本")
                
                st.write("") # 行間

                # 3段目: 走塁・その他 (盗塁, 得点)
                tc7, tc8, tc9 = st.columns(3)
                with tc7: show_top5("盗塁", df_bat_target, "盗塁", "Display", "盗塁", suffix=" 個")
                with tc8: show_top5("得点", df_bat_target, "得点", "Display", "得点", suffix=" 点")
                with tc9: st.write("") # 空白

                st.divider()
                st.markdown("####   🛡️   投手部門")
                tp1, tp2, tp3, tp4 = st.columns(4)
                with tp1:
                    filtered_c_era = df_pit_target[df_pit_target["Innings"] >= min_inn]
                    show_top5("防御率", filtered_c_era, "ERA", "Display", "ERA", ascending=True, suffix="", format_float=True)
                with tp2:
                    # ▼ WHIPを追加
                    filtered_c_whip = df_pit_target[df_pit_target["Innings"] >= min_inn]
                    show_top5("WHIP", filtered_c_whip, "WHIP", "Display", "WHIP", ascending=True, suffix="", format_float=True)
                with tp3: show_top5("勝利数", df_pit_target, "is_win", "Display", "is_win", suffix=" 勝")
                with tp4: show_top5("奪三振", df_pit_target, "is_so", "Display", "is_so", suffix=" 個")

            # --- 3. 個人年度別 ---
            with tab_personal:
                st.markdown("####   👤   個人年度別成績")
                st.caption("選手を選択すると、その選手の年度ごとの成績推移を表示します。")
                
                sel_player = st.selectbox("選手を選択", all_players, key="hist_player_sel")
                ...
                if sel_player:
                    ...
                    if not my_bat.empty:
                        ...
                        # 打率計算
                        bat_hist["打率"] = bat_hist.apply(lambda x: x["安打"] / x["打数"] if x["打数"] > 0 else 0.000, axis=1)
                        
                        # 並び替え
                        bat_hist = bat_hist.sort_index(ascending=False)
                        
                        # 通算行の作成
                        total_row = bat_hist.sum(numeric_only=True)
                        total_row.name = "通算"
                        # 率の再計算
                        if total_row["打数"] > 0:
                            total_row["打率"] = total_row["安打"] / total_row["打数"]
                        else:
                            total_row["打率"] = 0.0
                        
                        # 行結合
                        bat_hist = pd.concat([bat_hist, pd.DataFrame([total_row])])
                        
                        # フォーマット整形
                        disp_bat = bat_hist[["試合", "打率", "打数", "安打", "本塁打", "打点", "盗塁", "四死球", "三振"]].copy()
                    ...
                    if not my_pit.empty:
                        ...
                        pit_hist["frac"] = pit_hist["アウト数"] % 3
                        pit_hist["投球回"] = pit_hist.apply(lambda x: f"{int(x['回'])}.{int(x['frac'])}", axis=1)
                        ...
                        p_total.name = "通算"
                        p_total["回"] = p_total["アウト数"] // 3
                        p_total["frac"] = p_total["アウト数"] % 3
                        p_total_inn_str = f"{int(p_total['回'])}.{int(p_total['frac'])}"
                        
                        p_inn_float = p_total["アウト数"] / 3
                        if p_inn_float > 0:
                            p_era = (p_total["自責点"] * 7) / p_inn_float
                        else:
                            p_era = 0.0
                        
                        # 通算行をDataFrame化して結合 (投球回文字列などは手動セット)
        ...
        # ▼▼▼ 修正: タブを3つにする ▼▼▼
        t1, t2, t3 = st.tabs(["打撃成績", "投手成績", "守備記録"])

        # 共通の選択肢定義
        default_innings = [f"{i}回" for i in range(1, 10)] + ["延長", "試合終了"]
        ...
        batting_results = ["---", "単打", "二塁打", "三塁打", "本塁打", "三振", "四球", "死球", "犠打", "凡退", "失策", "盗塁", "得点", "走塁死", "盗塁死", "併殺打", "野選", "打撃妨害", "振り逃げ"]
        pitching_results = ["凡退", "三振", "単打", "二塁打", "三塁打", "本塁打", "四球", "死球", "失策", "犠打", "併殺打", "野選", "走塁死", "盗塁死", "牽制死"]
        ...
        with t2:
            ...
            ed_p = st.data_editor(
                ...
                column_config={
                    ...
                    # 処理野手は「選手名(位置)」の形式を保つため、自由入力(Text)にしておきます
                    "処理野手": st.column_config.TextColumn("処理野手", help="例: 中尾建太 (遊)"),
                    "球数": st.column_config.NumberColumn("球数", min_value=0, step=1),
                    "アウト数": st.column_config.NumberColumn("アウト数", min_value=0, max_value=2, step=1),
                    ...
                    "被安打": st.column_config.NumberColumn("被安打", min_value=0, max_value=1, step=1),
                },
                num_rows="dynamic",
                use_container_width=True,
                key="editor_pitching"
        ...
        # --- Tab 3: 守備記録 (プルダウン分解編集版) ---
        with t3:
            st.write("▼ 守備データの編集")
            st.caption("※ 「処理野手」は、編集しやすいように **「野手名」** と **「位置」** の2つに分けて表示しています。")
            st.caption("※ ここでの編集内容は「投手成績」にも反映されます。")

            # -----------------------------------------------------
            # 1. 前処理: 「処理野手」カラムを「名前」と「位置」に分解する
            # -----------------------------------------------------
            df_d_work = df_pitching.copy()

            # 分解用の関数
            def split_fielder_info(val):
                val_str = str(val)
                if "(" in val_str and ")" in val_str:
                    try:
                        ...
                        return val_str, ""
                return val_str, "" # カッコがない場合は名前だけ、位置は空

            # 新しい一時カラムを作成（データが空でない場合のみ実行）
            if not df_d_work.empty:
                # zip(*...)を使って2つのリストに展開して代入
                df_d_work["_temp_name"], df_d_work["_temp_pos"] = zip(*df_d_work["処理野手"].apply(split_fielder_info))
            else:
                df_d_work["_temp_name"] = ""
                df_d_work["_temp_pos"] = ""

            # -----------------------------------------------------
            # 2. 表示カラムの設定 (分解した2列を表示し、元の列は隠す)
            # -----------------------------------------------------
            # 表示順序
            d_column_order = [
                "日付", "対戦相手", "イニング", "投手名", "結果",
                "_temp_name", "_temp_pos", # 分解した2列をここに配置
                "失策", "併殺打",
                "試合種別", "グラウンド"
            ]
            
            # その他のカラム（表示順序指定外のもの）
            valid_cols = [c for c in d_column_order if c in df_d_work.columns]
            other_cols = [c for c in df_d_work.columns if c not in valid_cols and c != "処理野手"]

            # エディタ設定
            ed_d = st.data_editor(
                df_d_work,
                column_order=valid_cols + other_cols,
                column_config={
                    "日付": st.column_config.DateColumn("日付", format="YYYY-MM-DD", required=True),
                    "グラウンド": st.column_config.SelectboxColumn("グラウンド", options=merged_grounds),
                    "対戦相手": st.column_config.SelectboxColumn("対戦相手", options=merged_opps),
                    "試合種別": st.column_config.SelectboxColumn("試合種別", options=match_types),
                    "イニング": st.column_config.SelectboxColumn("イニング", options=default_innings),
                    "投手名": st.column_config.SelectboxColumn("投手名", options=all_players),
                    "結果": st.column_config.SelectboxColumn("結果", options=pitching_results, required=True),
                    
                    # ▼▼▼ 分解した2つのカラムをプルダウン化 ▼▼▼
                    "_temp_name": st.column_config.SelectboxColumn(
                        "処理野手(名)", 
                        options=all_players, 
                        help="野手の名前を選択"
                    ),
                    "_temp_pos": st.column_config.SelectboxColumn(
                        "守備位置", 
                        options=["", "投", "捕", "一", "二", "三", "遊", "左", "中", "右"],
                        help="守備位置を選択（空欄も可）"
                    ),
                    # 元の「処理野手」カラムは隠す（column_orderに入れていないので自動で隠れるが念のため）
                    "処理野手": None,

                    "失策": st.column_config.NumberColumn("失策", min_value=0, max_value=1, step=1),
                    "併殺打": st.column_config.NumberColumn("併殺打", min_value=0, max_value=1, step=1),
                    
                    # 不要な数値の編集制限
                    "失点": st.column_config.NumberColumn("失点", disabled=True),
                    "自責点": st.column_config.NumberColumn("自責点", disabled=True),
                },
                num_rows="dynamic",
                use_container_width=True,
                key="editor_defense_split"
            )

            if st.button("守備記録を保存", type="primary"):
                # -----------------------------------------------------
                # 3. 保存処理: 2つのカラムを合体させて「処理野手」に戻す
                # -----------------------------------------------------
                # 保存用のDataFrameを準備
                df_to_save = ed_d.copy()
                
                def reconstruct_fielder_str(row):
                    name = str(row.get("_temp_name", ""))
                    pos = str(row.get("_temp_pos", ""))
                ...
                # 合体処理
                df_to_save["処理野手"] = df_to_save.apply(reconstruct_fielder_str, axis=1)

                # 一時カラムを削除
                if "_temp_name" in df_to_save.columns:
                    df_to_save = df_to_save.drop(columns=["_temp_name", "_temp_pos"])

                # Google Sheetsを更新
                conn.update(spreadsheet=SPREADSHEET_URL, worksheet="投手成績", data=df_to_save)
                st.cache_data.clear()
                st.success(" ✅  守備データを更新しました！")
                import time
                time.sleep(1.0)
                st.rerun()